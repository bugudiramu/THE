  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 className="text-4xl font-extrabold text-gray-900 mb-10 text-center">Our Fresh Essentials</h1>
        ...
        {products.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {products.map((product) => {
              const mainImage = product.images?.[0]?.url || "https://images.unsplash.com/photo-1518569656558-1f25e69c93d7?w=800&h=600&fit=crop";
              const inCartQty = getInCartQuantity(product.id);
              
              return (
                <div key={product.id} className="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col border border-gray-100 relative">
                  {inCartQty > 0 && (
                    ...
                    <div className="mt-auto pt-6 border-t border-gray-50 flex items-center justify-between">
                      <div className="flex flex-col">
                        ...
                        {product.subPrice && (
                          <span className="text-sm text-green-600 font-medium">
                            Save Rs. {((product.price - product.subPrice)/100).toFixed(2)} with sub
                          </span>
                        )}
                      </div>
                      ...
                      {inCartQty > 0 ? (
                        <div className="flex items-center bg-gray-100 rounded-xl p-1 border">
                           ...
                           >-</button>
                           <span className="w-8 text-center font-bold text-sm">{inCartQty}</span>
                           <button 
                             onClick={() => {
                               const item = items.find(i => i.productId === product.id);
